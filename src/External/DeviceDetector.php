...
class DeviceDetector implements DetectorInterface
{
	protected static $androidList = [
		'PC','OPPO','RedMi','XiaoMi','Vivo','Samsung','HuaWei','Letv','Lenovo','HTC','Meizu','ZTE','ONEPLUS'
	];
    /**
    ...
    public static function detect(Device $device, UserAgent $userAgent)
    {
        ...
        /*iphone，ipad，windowphone pc*/
        $flag = self::checkIpad($device, $userAgent) ||self::checkIphone($device, $userAgent) || self::checkWindowsPhone($device, $userAgent) || self::checkPc($device, $userAgent);
        if( $flag ){
        	return true;
		}
        /*android*/
    ...
    /**
     * Determine if the device is Pc in('Windows NT ', 'Mac OS X ').
     *
     * @param Device $device
     * @param UserAgent $userAgent
     * @return bool
     */
    private static function checkPc(Device $device, UserAgent $userAgent)
    {
        if (stripos($userAgent->getUserAgentString(), 'Windows NT ') !== false || stripos($userAgent->getUserAgentString(), 'Mac OS X ') !== false) {
